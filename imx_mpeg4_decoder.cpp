/*
 * Module imx_mpeg4_decoder.cpp
 *
 * This module defines the methods of the mpeg4_decoder_t class
 * as declared in imx_mpeg4_decoder.h
 *
 * Copyright Boundary Devices, Inc. 2010
 */

#include "imx_mpeg4_decoder.h"
#include <stdlib.h>
#include <stdio.h>
#include <unistd.h>
#include <string.h>
...
mpeg4_decoder_t::mpeg4_decoder_t(vpu_t &vpu)
	: w_(0)
	, h_(0)
	, ystride_(0)
	, uvstride_(0)
	...
	, state_(ERR)
{
	...
	oparam.bitstreamFormat = STD_MPEG4 ;
...
mpeg4_decoder_t::~mpeg4_decoder_t(void) {
	if(startedDecode_){
		vpu_WaitForInt(0);
	}
...
bool mpeg4_decoder_t::space_avail( unsigned &length )
{
	RetCode ret;
...
bool mpeg4_decoder_t::fill_buf( void const *data, unsigned length, void *opaque )
{
	RetCode ret;
	PhysicalAddress pa_read_ptr, pa_write_ptr;
	unsigned long space;
...
void mpeg4_decoder_t::startDecode(void)
{
...
#endif
			DecParam decparam = {0};
			RetCode ret = vpu_DecStartOneFrame(handle_, &decparam);
...
bool mpeg4_decoder_t::decode_complete(void const *&y, void const *&u, void const *&v, void *&opaque, unsigned &displayIdx, int &picType, bool &errors )
{
        displayIdx = 0 ;
	errors = false ;
...
void mpeg4_decoder_t::releaseBuffer(unsigned displayIdx)
{
