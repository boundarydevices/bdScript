        ...
        /*
            pix.bytesperline = ystride_= (inw+15)&~15 ;
                ...
                if( V4L2_PIX_FMT_NV12 == pix.pixelformat ){
                        off.u_offset = ysize_ ;
                        off.v_offset = ysize_ ;
                } else if( V4L2_PIX_FMT_YUV420 == pix.pixelformat ){
                        off.u_offset = ysize_ ;
                        off.v_offset = ysize_ + uvsize_ ;
                } else {
                        ERRMSG("unsupported pixel format %d\n", pix.pixelformat);
                }
                fmt.fmt.pix.priv = (unsigned long) &off;
        */
        struct v4l2_mxc_offset off ; memset(&off,0,sizeof(off));
        switch (pix.pixelformat) {
                case V4L2_PIX_FMT_YUV420:
                        pix.bytesperline = ystride_ = (inw + 15) & ~15;
                        off.u_offset = ysize_ = pix.bytesperline * inh;
                        pix.sizeimage = off.u_offset * 3 / 2;
                        uvsize_ = (off.u_offset / 4);
                        off.v_offset = off.u_offset + uvsize_;
                        fmt.fmt.pix.priv = (unsigned long) &off;
                        break;
                ...
                case V4L2_PIX_FMT_YUV422P:
                        ...
                        uvsize_ = (off.u_offset / 2);
                        off.v_offset = off.u_offset + uvsize_;
                        fmt.fmt.pix.priv = (unsigned long) &off;
                        break;
                case V4L2_PIX_FMT_UYVY:
                        pix.bytesperline = ystride_ = ((inw << 1) + 15) & ~15;
                        fmt.fmt.pix.priv = 0;
                        ysize_ = 0 ;
                        uvsize_ = 0 ;
                        pix.bytesperline = ((inw << 1) + 15) & ~15;
                        pix.sizeimage = inw * inh;
                        break;
                default:
                        ERRMSG("unknown pixel format 0x%08x\n",pix.pixelformat);
        }
        uvstride_ = ystride_ / 2 ;

        debugPrint( "set format: %ux%u (%u bytes) -> %ux%u: offsets %u/%u, pixelformat %08x\n", 
                    inw, inh, pix.sizeimage, outw, outh, off.u_offset,off.v_offset,pix.pixelformat );

        err = ioctl(fd_, VIDIOC_S_FMT, &fmt);
        if (err < 0) {
                ERRMSG("VIDIOC_S_FMT failed: %s\n",strerror(errno));
                close();
                return ;
        }

        int out = destination_ ;
        ...
        if (err < 0) {
                ...
                ERRMSG("-------------------------------------------------------- set to YUV overlay %d\n", out );

        debugPrint( "set output to /dev/graphics/fb%d\n", (out-3)*2 );
        ...
        if (ioctl(fd_fb, MXCFB_SET_CLR_KEY, &key) <0) {
                ERRMSG("MXCFB_SET_CLR_KEY error!");
                ::close(fd_fb);
                close();
                return ;
        }
        ...
        debugPrint( "set global alpha to %u\n", transparency );
}
...
void v4l_overlay_t::putBuf( unsigned idx )
{
        ...
        int err = ioctl(fd_, VIDIOC_QUERYBUF, &buffer);
        if (err < 0) {
                ERRMSG("%s:VIDIOC_QUERYBUF: not enough buffers\n",__func__);
                return ;
        }
        ...
        buffer.sequence = 0 ;
        err = ioctl(fd_, VIDIOC_QBUF, &buffer);
        if ( err < 0) {
                ERRMSG("VIDIOC_QBUF");
                return ;
        }
...
int main( int argc, char const **argv ) {
        ...
        printf( "%ux%u -> %ux%u (alpha %u), device %s\n", inwidth, inheight, outwidth, outheight, alpha, devName );

        v4l_overlay_t overlay(inwidth,inheight,x,y,outwidth,outheight,alpha,v4l_overlay_t::outputOverlay,V4L2_PIX_FMT_NV12); // V4L2_PIX_FMT_SGRBG8 ; // 
        if ( overlay.isOpen() ) {
                ...
                while ( fgets(inBuf,sizeof(inBuf),stdin) ) {
                        ...
                        if ( '\0' == c ) {
                                ...
                                if ( overlay.getBuf(index,y,u,v) ) {
                                        printf( "y:%p/%u/%u\n", y,yval,overlay.ySize());
                                        memset(y,yval,overlay.ySize());
                                        if ( u+overlay.uvSize() <= v ) {
                                                printf( "u:%p/%u/%u\n", u,uval,overlay.uvSize());
                                                memset(u,uval,overlay.uvSize());
                                                printf( "v:%p/%u/%u\n", v,vval,overlay.uvSize());
                                                memset(v,vval,overlay.uvSize());
                                        }
                                        else {
                                                printf("uv:%p/%p(%u) == %u/%u\n", u,v,overlay.uvSize(),uval,vval);
                                                for ( unsigned i = 0 ; i < overlay.uvSize(); i++ ) {
                                                        *u = uval ; u += 2 ;
                                                        *v = vval ; v += 2 ;
                                                }
                                        }
