...
void ScaleYCbCrf2::DoLineJPEGf2(BYTE* dest,DWORD* yPtr,DWORD* cbPtr,DWORD* crPtr,int colsLeft,
...
//B = Y + (.86100*Cb)*2 - 220.16
		ULONGLONG yData = (yMult)? (static_cast<ULONGLONG>(yMult)*y2) : (static_cast<ULONGLONG>(y2) <<32);
		ULONGLONG tval = yData + ((static_cast<ULONGLONG>(cbBlueMult)*(cb1+cb2+cb3))<<1) - ((static_cast<ULONGLONG>(220)<<32) + 687194767);
...
//R = Y          + (.70100*Cr<<1) - 179.456
		yData = (yMult)? (static_cast<ULONGLONG>(yMult)*y4) : (static_cast<ULONGLONG>(y4) <<32);
		tval = yData + ((static_cast<ULONGLONG>(crRedMult)*(crr3+crr4+crr5))<<1) - ((static_cast<ULONGLONG>(179) << 32) + 1958505087);
		val = static_cast<int>(tval >> 32);
		frac = static_cast<int>(tval);
		if (frac<0) val++;
		if (((DWORD)val)>255)
...
// ********************
		...
		mul		ecx
		mov		ecx,edx
	z1:	mov		ebx,eax			//ULONGLONG yData = (yMult)? (static_cast<ULONGLONG>(yMult)*y2) : (static_cast<ULONGLONG>(y2) <<32);
		...
		adc		edx,ecx
		sub		eax,687194767
		sbb		edx,220			//ULONGLONG tval = yData + ((static_cast<ULONGLONG>(cbBlueMult)*(cb1+cb2+cb3))<<1) - ((static_cast<ULONGLONG>(220)<<32) + 687194767);
		add		eax,eax
		mov		eax,edx			//int val = tval >> 32;
		adc		eax,0			//if (frac<0) val++;
		cmp		eax,256			//if (((DWORD)val)>255)
		setc	dl				//(eax<256) ? 1 : 0
...
// ********************
	...
	z2:	mov		ebx,eax				//yData = (yMult)? (static_cast<ULONGLONG>(yMult)*y3) : (static_cast<ULONGLONG>(y3) <<32);
		...
		mul		edx
		sub		ebx,eax
		sbb		ecx,edx				//tval = (yData + ((static_cast<ULONGLONG>(135) << 32) + 1974997761)) - static_cast<ULONGLONG>(cbGreenMult)*(cb2+cb3+cb4) - static_cast<ULONGLONG>(crGreenMult)*(cr2+cr3+cr4)
...
// ********************
		...
		mul		ecx
		mov		ecx,edx
	z3:	mov		ebx,eax				//yData = (yMult)? (static_cast<ULONGLONG>(yMult)*y4) : (static_cast<ULONGLONG>(y4) <<32);
		...
		adc		edx,ecx
		sub		eax,1958505087
		sbb		edx,179				//tval = yData + ((static_cast<ULONGLONG>(crRedMult)*(crr3+crr4+crr5))<<1) - ((static_cast<ULONGLONG>(179) << 32) + 1958505087)
		add		eax,eax
...
void ScaleYCbCrf2::CreateBitmap(ScaleUpObj* yObj,ScaleUpObj* cbObj,ScaleUpObj* crObj,DWORD srcWidth,DWORD rowCnt,BYTE* dest,DWORD rowWidth)
{
	...
	while (rowCnt--)
	{
		DoLineJPEGf2(dest,yObj->SampleRow(),cbObj->SampleRow(),crObj->SampleRow(),srcWidth,
